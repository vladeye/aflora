	...
	for Category in CategoryArray[0]:
		...
		if CategoryParentID.text == CategoryID.text:
			if BestOfferEnabled is None:
				cursor = conn.cursor()
				cursor.execute('SELECT COUNT(*) FROM CATEGORY WHERE Id = ?', (CategoryID.text,))
...
def getParentCategories(conn):
	...
	if len(arrData) == 0:
		sys.exit("Categories parents don't exist")
	return arrData 	
...
xml_datarsp = xml_data.replace("{@replace@}",xml_level)
req = getReq(xml_datarsp)
downloadChunks(req)	
fillTable(conn)
arrParentCat = getParentCategories(conn)
for row in arrParentCat:
	print "Category Parent: " + str(row[0])
	xml_catpar = '<CategoryParent>' + str(row[0]) + '</CategoryParent>' 
	xml_datarsp = xml_data.replace("{@replace@}",xml_catpar)
	req = getReq(xml_datarsp)		
	downloadChunks(req)	
	fillTable(conn)
conn.close()
